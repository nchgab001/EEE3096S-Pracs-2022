...
int main(void){
  ...
  while (1)
  {
	...
	sprintf(buffer, "Time = %dh, %dm, %ds \r",time.hour, time.minutes, time.seconds);
...
void pause_sec(float x)
{
	...
	int cycPerSec = 3197500;
	int finalVal = x * cycPerSec;
	int count = 0;
...
void setTime (uint8_t sec, uint8_t min, uint8_t hour, uint8_t dow, uint8_t dom, uint8_t month, uint8_t year)
{
	...
	HAL_I2C_Mem_Write(&hi2c1, 0xD0, 0x00, I2C_MEMADD_SIZE_8BIT, set_time, 7, 1000);
...
void getTime (void)
{
	...
	HAL_I2C_Mem_Read(&hi2c1, 0xD0, 0x00, I2C_MEMADD_SIZE_8BIT, get_time, 7, 1000);
...
int epochFromTime(TIME time){
	...
	return EPOCH_2022 + day * (60*60*24) + hour * (60*60) + minutes * (60) + seconds;
}
