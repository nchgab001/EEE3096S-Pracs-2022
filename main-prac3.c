...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  uint32_t crr_val = ADCtoCRR(adcVal);
	  ...
	  uint32_t dutyCycle = (adcVal*100)/4095;
...
uint32_t pollADC(void){
	...
	uint32_t val = HAL_ADC_GetValue(ADC1);
	HAL_ADC_Stop(ADC1);
	HAL_Delay(1);*/
...
uint32_t ADCtoCRR(uint32_t adc_val){
	...
	float scaleFactor = 47999/4095;
	uint32_t val = adc_val * scaleFactor;
	return val;
}
